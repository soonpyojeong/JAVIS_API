...
public class TbService {
    ...
    public List<TsSummaryDto> getTablespaceSummary(TsSummaryRequestDto request) {
        String sql = "WITH\n" +
                ...
                "  WHERE CHK_DATE BETWEEN REPLACE(:startDate,'-','') AND REPLACE(:endDate,'-','')\n" +
                "    AND DB_NAME = :dbName\n" +
                ...
                "\n" +
                "    (SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY x.inc_mb_for_avg)   -- 중앙값 0.5로 보정\n" +
                "       FROM growth2 x\n" +
                "      WHERE x.DB_NAME=p.DB_NAME AND x.TS_NAME=p.TS_NAME\n" +
                "        AND x.dt > p.dt - (SELECT post_window_days FROM params)\n" +
                "        AND x.dt <= p.dt\n" +
                ...
                "         MIN(used_day_mb) AS min_used_mb,\n" +
                "         MAX(used_day_mb) AS max_used_mb\n" +
                "  FROM daily\n" +
                "  GROUP BY DB_NAME, TS_NAME\n" +
                ")\n" +
                "\n" +
                ...
                "  f.DB_NAME,\n" +
                "  f.TS_NAME,\n" +
                "  f.cap_asof_mb                              AS TOTAL_SIZE_MB,      -- DTO 별칭 통일\n" +
                "  (u.max_used_mb - u.min_used_mb)            AS REAL_USED_MB,\n" +
                ...
                "  /* REAL_USED_PERCENT: 해당 TS 실사용 증가가 DB 전체 증가에서 차지하는 비중(%) */\n" +
                "  ROUND( (u.max_used_mb - u.min_used_mb)\n" +
                "         / NULLIF( SUM(u.max_used_mb - u.min_used_mb) OVER (PARTITION BY f.DB_NAME), 0 ) * 100, 2) AS REAL_USED_PERCENT,\n" +
                "  CASE\n" +
                "    WHEN NVL(f.speed_final,0) <= 0 THEN NULL\n" +
                "    WHEN NVL(lv.curr_max_free_mb, lv.curr_free_mb) <= 0 THEN 0\n" +
                "    ELSE CEIL( NVL(lv.curr_max_free_mb, lv.curr_free_mb) / f.speed_final )\n" +
                "  END AS DAYS_TO_FULL,\n" +
                "  CASE\n" +
                "    WHEN lv.curr_used_mb >= (f.cap_asof_mb * 0.95) THEN 0\n" +
                "    WHEN NVL(f.speed_final,0) <= 0 THEN NULL\n" +
                "    ELSE CEIL( GREATEST(ROUND(f.cap_asof_mb*0.95) - lv.curr_used_mb, 0) / f.speed_final )\n" +
                "  END AS DAYS_TO_95PERCENT\n" +
                "FROM final_speed f\n" +
                "JOIN last_vals lv ON lv.DB_NAME=f.DB_NAME AND lv.TS_NAME=f.TS_NAME AND lv.last_dt=f.dt\n" +
                "JOIN agg_usage u ON u.DB_NAME=f.DB_NAME AND u.TS_NAME=f.TS_NAME" ;
        ...
        return namedJdbcTemplate.query(sql, params, (rs, rowNum) ->
                new TsSummaryDto(
                        ...
                        rs.getDouble("TOT_USAGE_PERCENT"),   // long → double 권장
                        rs.getDouble("REAL_USED_MB"),
                        rs.getDouble("REAL_USED_PERCENT"),
                        rs.getDouble("REMAIN_MB"),
                        rs.getLong("DAYS_TO_FULL"),
                        rs.getLong("DAYS_TO_95PERCENT")
                ));
    }
}
